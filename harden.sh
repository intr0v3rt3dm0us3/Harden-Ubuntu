...
if prompt_yn "Create an admin user?"; then
    ...
    echo "  This password is the last line of defense if someone gains SSH access."
    echo "  Every 'sudo' command requires it — package installs, config changes,"
    echo "  firewall rules, service restarts. A weak password here means an attacker"
    echo "  with shell access is one guess away from full root control."
    echo ""
    echo -e "  ${BOLD}What makes a strong password:${NC}"
    echo "    ✓ At least 16 characters (longer = exponentially harder to crack)"
    echo "    ✓ Mix of uppercase, lowercase, numbers, and symbols"
    echo "    ✓ Randomly generated — not based on words, names, or patterns"
    echo "    ✓ Unique to this server — never reused from another account"
    echo ""
    echo -e "  ${BOLD}What to avoid:${NC}"
    echo "    ✗ Dictionary words (even with l33t substitutions like P@ssw0rd)"
    echo "    ✗ Personal info (birthdays, pet names, addresses)"
    echo "    ✗ Keyboard patterns (qwerty, 123456, zxcvbn)"
    echo "    ✗ Short passwords (<12 chars can be brute-forced in hours)"
    echo ""
    echo -e "  ${BOLD}Recommended approach:${NC}"
    echo "  Use a password manager to generate and store a random password."
    echo "  Bitwarden has a free online generator:"
    echo -e "  ${CYAN}https://bitwarden.com/password-generator/${NC}"
    echo ""
    echo "  If you prefer a memorable passphrase, use 4+ random words:"
    echo "    correct-horse-battery-staple    (good: 4 random words)"
    echo "    purple-volcano-bicycle-fox-91   (better: 5 words + number)"
    echo ""
    while true; do
